...
type Post = {
  id: string; user: string; avatar: string; time: string; following: boolean;
  text: string; images: string[]; video?: string; videoThumbnail?: string;
  mediaAspect?: MediaAspect; views?: string; likes: number; comments: number;
  shares: number; type: 'post' | 'ad'; adTitle?: string; adSubtitle?: string;
  adBrand?: string; adCta?: string;
};
...
const POSTS: Post[] = [
  {
    ...
    mediaAspect: 'landscape', views: '999K', likes: 112, comments: 35, shares: 127, type: 'post',
  },
  {
  ...
  {
    id: '3', user: 'Ananya Rao', avatar: 'https://i.pravatar.cc/150?img=47',
    ...
    mediaAspect: 'portrait', views: '2.4M', likes: 4821, comments: 312, shares: 890, type: 'post',
  },
  {
    id: '4', user: 'Priya Kapoor', avatar: 'https://i.pravatar.cc/150?img=45',
    ...
    mediaAspect: 'square', likes: 540, comments: 48, shares: 19, type: 'post',
  },
];

// ─── Blood Donation Card ──────────────────────────────────────────────────────

const BloodDonationBanner = ({ onIgnore, onAccept }: { onIgnore: () => void; onAccept: () => void }) => (
  <View style={bdStyles.card}>
    {/* Decorative circles */}
    <View style={bdStyles.decCircle1} pointerEvents="none" />
    <View style={bdStyles.decCircle2} pointerEvents="none" />

    {/* Top row */}
    <View style={bdStyles.topRow}>
      <Image source={{ uri: 'https://i.pravatar.cc/150?img=11' }} style={bdStyles.avatar} />
      <View style={bdStyles.textBlock}>
        <Text style={bdStyles.title}>You are the nearby match</Text>
        <Text style={bdStyles.desc}>
          {'Rahul rise blood donation request for Raghini Mehra\n>> Blood Group type '}
          <Text style={bdStyles.boldText}>AB-</Text>
        </Text>
      </View>
    </View>

    {/* Pills */}
    <View style={bdStyles.pillsRow}>
      <View style={bdStyles.pill}>
        <Text style={bdStyles.pillIcon}>🩸</Text>
        <Text style={bdStyles.pillText}>1 Unit</Text>
      </View>
      <View style={bdStyles.pill}>
        <Text style={bdStyles.pillIcon}>📍</Text>
        <Text style={bdStyles.pillText}>Apollo Hospital, Noida</Text>
      </View>
      <View style={bdStyles.pill}>
        <Text style={bdStyles.pillIcon}>🕐</Text>
        <Text style={bdStyles.pillText}>ASAP</Text>
      </View>
    </View>

    {/* Buttons */}
    <View style={bdStyles.buttonsRow}>
      <TouchableOpacity style={bdStyles.ignoreBtn} onPress={onIgnore} activeOpacity={0.85}>
        <Text style={bdStyles.ignoreBtnTitle}>Ignore</Text>
        <Text style={bdStyles.ignoreBtnSub}>We will not notify them</Text>
      </TouchableOpacity>
      <TouchableOpacity style={bdStyles.acceptBtn} onPress={onAccept} activeOpacity={0.85}>
        <Text style={bdStyles.acceptBtnText}>Accept</Text>
      </TouchableOpacity>
    </View>
  </View>
);
...
const bdStyles = StyleSheet.create({
  card: {
    ...
    marginBottom: 0, gap: 14, overflow: 'hidden',
  },
  decCircle1: {
    position: 'absolute', width: 160, height: 160, borderRadius: 80,
    backgroundColor: 'rgba(255,150,150,0.18)', top: -40, right: -30,
  },
  decCircle2: {
    position: 'absolute', width: 100, height: 100, borderRadius: 50,
    backgroundColor: 'rgba(255,150,150,0.15)', bottom: -20, left: 60,
  },
  topRow:    { flexDirection: 'row', alignItems: 'flex-start', gap: 12 },
  avatar:    { width: 64, height: 64, borderRadius: 10, flexShrink: 0 },
  textBlock: { flex: 1 },
  title:     { fontSize: 16, fontFamily: 'SofiaSansCondensed-Bold', color: '#1a1a1a', marginBottom: 4 },
  desc:      { fontSize: 15, fontFamily: 'SofiaSansCondensed-Regular', color: '#333', lineHeight: 19 },
  boldText:  { fontFamily: 'SofiaSansCondensed-Bold', color: '#1a1a1a' },
  ...
  pillText: { fontSize: 14, fontFamily: 'SofiaSansCondensed-Regular', color: '#1a1a1a' },
  buttonsRow: { flexDirection: 'row', gap: 10 },
  ignoreBtn: {
    flex: 1, backgroundColor: '#b03030', borderRadius: 12,
    paddingVertical: 12, alignItems: 'center', justifyContent: 'center',
  },
  ignoreBtnTitle: { fontSize: 15, fontFamily: 'SofiaSansCondensed-SemiBold', color: '#fff' },
  ignoreBtnSub:   { fontSize: 14, fontFamily: 'SofiaSansCondensed-Regular', color: 'rgba(255,255,255,0.8)', marginTop: 1 },
  acceptBtn: {
    flex: 1, backgroundColor: '#8b7d2a', borderRadius: 12,
    paddingVertical: 12, alignItems: 'center', justifyContent: 'center',
  },
  acceptBtnText: { fontSize: 15, fontFamily: 'SofiaSansCondensed-SemiBold', color: '#fff' },
});
...
const BIRTHDAY_PEOPLE = [
  { id: 'b1', name: 'Sohan',  avatar: 'https://i.pravatar.cc/150?img=13' },
  { id: 'b2', name: 'Priya',  avatar: 'https://i.pravatar.cc/150?img=45' },
  { id: 'b3', name: 'Aarav',  avatar: 'https://i.pravatar.cc/150?img=15' },
];

// Single swipeable birthday card (one person)
const BirthdayCard = ({
  person,
  totalCount,
  currentIndex,
  ...
  person: typeof BIRTHDAY_PEOPLE[0];
  totalCount: number;
  currentIndex: number;
  onIgnore: () => void;
  onSendWishes: (wish: string) => void;
}) => {
  const [selectedWish, setSelectedWish] = useState<string | null>(null);

  return (
    <View style={[bStyles.card, { width }]}>
      {/* Decorative dots */}
      <View style={bStyles.dot1} pointerEvents="none" />
      <View style={bStyles.dot2} pointerEvents="none" />
      <View style={bStyles.dot3} pointerEvents="none" />

      {/* Top row */}
      <View style={bStyles.topRow}>
        <Image source={{ uri: person.avatar }} style={bStyles.avatar} />
        <View style={bStyles.textBlock}>
          <Text style={bStyles.title}>{`It's ${person.name}'s birthday today!`}</Text>
          <Text style={bStyles.desc}>
            {'Make it even more special by sending him some warm wishes.'}
          </Text>
        </View>
        {/* Pagination dots */}
        <View style={bStyles.dotsRow}>
          {Array.from({ length: totalCount }).map((_, i) => (
            <View
              key={i}
              style={[
                bStyles.dotInd,
                { backgroundColor: i === currentIndex ? '#e53935' : '#e0e0e0' },
              ]}
            />
          ))}
        </View>
      ...
      {/* Quick reply chips */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={bStyles.chipsRow}>
        {BIRTHDAY_WISHES.map((w, i) => (
          <TouchableOpacity
            key={i}
            style={[bStyles.chip, selectedWish === w && bStyles.chipActive]}
            onPress={() => setSelectedWish(w)}
            activeOpacity={0.75}
          >
            <Text style={[bStyles.chipText, selectedWish === w && bStyles.chipTextActive]}>{w}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>

      {/* Buttons */}
      <View style={bStyles.buttonsRow}>
        <TouchableOpacity style={bStyles.ignoreBtn} onPress={onIgnore} activeOpacity={0.85}>
          <Text style={bStyles.ignoreBtnText}>Ignore</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[bStyles.sendBtn, selectedWish ? bStyles.sendBtnActive : null]}
          onPress={() => selectedWish && onSendWishes(selectedWish)}
          activeOpacity={0.85}
        >
          <Text style={[bStyles.sendBtnText, selectedWish ? bStyles.sendBtnTextActive : null]}>
            Sent Wishes
          </Text>
        </TouchableOpacity>
      </View>
...
// Carousel wrapper — swipe horizontally through all birthday cards
const BirthdayBanner = ({
  onAllDone,
}: {
  onAllDone: () => void;
}) => {
  const [people, setPeople] = useState(BIRTHDAY_PEOPLE);
  const [activeIndex, setActiveIndex] = useState(0);
  const flatRef = useRef<FlatList>(null);

  const handleIgnore = (id: string) => {
    const next = people.filter(p => p.id !== id);
    if (next.length === 0) { onAllDone(); return; }
    setPeople(next);
    const newIdx = Math.min(activeIndex, next.length - 1);
    setActiveIndex(newIdx);
    setTimeout(() => flatRef.current?.scrollToIndex({ index: newIdx, animated: true }), 50);
  };

  const handleSendWishes = (id: string, wish: string) => {
    Alert.alert('Wishes Sent! 🎂', `You sent: "${wish}"`);
    handleIgnore(id); // advance to next card after sending
  };
    ...
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <BirthdayCard
          ...
          onSendWishes={wish => handleSendWishes(item.id, wish)}
        />
      )}
    />
  );
...
const bStyles = StyleSheet.create({
  ...
  // Decorative dots
  dot1: { position: 'absolute', width: 18, height: 18, borderRadius: 9, backgroundColor: '#e53935', top: 10, right: 60, opacity: 0.7 },
  dot2: { position: 'absolute', width: 12, height: 12, borderRadius: 6, backgroundColor: '#fff176', top: 30, right: 30, opacity: 0.8 },
  dot3: { position: 'absolute', width: 10, height: 10, borderRadius: 5, backgroundColor: '#81c784', bottom: 20, right: 80, opacity: 0.7 },
  topRow:    { flexDirection: 'row', alignItems: 'flex-start', gap: 12 },
  avatar:    { width: 64, height: 64, borderRadius: 10, flexShrink: 0 },
  textBlock: { flex: 1 },
  title:     { fontSize: 16, fontFamily: 'SofiaSansCondensed-Bold', color: '#1a1a1a', marginBottom: 4 },
  desc:      { fontSize: 15, fontFamily: 'SofiaSansCondensed-Regular', color: '#1a1a1a', lineHeight: 18, opacity: 0.85 },
  dotsRow:   { flexDirection: 'row', gap: 4, alignItems: 'center', paddingTop: 2 },
  dotInd:    { width: 8, height: 8, borderRadius: 4 },
  ...
  chipTextActive: { fontFamily: 'SofiaSansCondensed-Bold', color: '#1a1a1a' },
  buttonsRow: { flexDirection: 'row', gap: 10 },
  ignoreBtn: {
    flex: 1, backgroundColor: '#e53935', borderRadius: 12,
    paddingVertical: 14, alignItems: 'center',
  },
  ignoreBtnText: { fontSize: 15, fontFamily: 'SofiaSansCondensed-SemiBold', color: '#fff' },
  sendBtn: {
    flex: 1, backgroundColor: 'rgba(180,180,180,0.5)', borderRadius: 12,
    paddingVertical: 14, alignItems: 'center',
  },
  sendBtnActive:    { backgroundColor: '#c8b560' },
  sendBtnText:      { fontSize: 15, fontFamily: 'SofiaSansCondensed-SemiBold', color: 'rgba(255,255,255,0.6)' },
  sendBtnTextActive:{ color: '#fff' },
});

// ─── Save Toast ───────────────────────────────────────────────────────────────

type SaveToastProps = { visible: boolean; postImage?: string; onHide: () => void };
...
const SaveToast = ({ visible, postImage, onHide }: SaveToastProps) => {
  ...
  return (
    <Animated.View style={[toastStyles.container, { transform: [{ translateY: slideAnim }], opacity: opacityAnim }]}>
      {postImage
        ? <Image source={{ uri: postImage }} style={toastStyles.thumbnail} resizeMode="cover" />
        : <View style={toastStyles.thumbnailPlaceholder}><Text style={{ fontSize: 18 }}>🔖</Text></View>
      }
      ...
      <View style={toastStyles.checkCircle}><Text style={toastStyles.checkMark}>✓</Text></View>
    </Animated.View>
  );
};
...
const toastStyles = StyleSheet.create({
  container: {
    ...
    shadowOpacity: 0.25, shadowRadius: 12, elevation: 10, zIndex: 9999,
  },
  thumbnail: { width: 44, height: 44, borderRadius: 10, backgroundColor: '#333' },
  thumbnailPlaceholder: { width: 44, height: 44, borderRadius: 10, backgroundColor: '#333', justifyContent: 'center', alignItems: 'center' },
  textWrap: { flex: 1 },
  title:    { fontSize: 14, color: '#fff', fontFamily: 'SofiaSansCondensed-Bold', marginBottom: 2 },
  subtitle: { fontSize: 12, color: '#aaa', fontFamily: 'SofiaSansCondensed-Regular' },
  checkCircle: { width: 28, height: 28, borderRadius: 14, backgroundColor: 'rgba(255,167,87,1)', justifyContent: 'center', alignItems: 'center' },
  checkMark:   { color: '#fff', fontSize: 14, fontWeight: '700' },
});

// ─── RichText ─────────────────────────────────────────────────────────────────

const RichText = ({ text, style, onHashtagPress, onMentionPress }: {
  text: string; style?: any; onHashtagPress?: (tag: string) => void; onMentionPress?: (user: string) => void;
}) => {
  const tokens = text.split(/([@#][\w\u00C0-\u024F]+)/g);
  return (
    <Text style={style}>
      {tokens.map((token, i) => {
        if (token.startsWith('#')) return <Text key={i} style={rtStyles.hashtag} onPress={() => onHashtagPress?.(token.slice(1))}>{token}</Text>;
        if (token.startsWith('@')) return <Text key={i} style={rtStyles.mention} onPress={() => onMentionPress?.(token.slice(1))}>{token}</Text>;
...
const rtStyles = StyleSheet.create({
  hashtag: { color: 'rgba(255,167,87,1)', fontFamily: 'SofiaSansCondensed-Medium' },
  mention: { color: 'rgba(0,107,165,1)', fontFamily: 'SofiaSansCondensed-Medium' },
});

// ─── Post Options Sheet ───────────────────────────────────────────────────────

const PostOptionsSheet = ({ visible, onClose, onSave, onShare }: {
  ...
  const handleAction = (action: () => void) => { onClose(); setTimeout(action, 260); };
...
const ContextMenu = ({ visible, onClose, onReport, onBlock, onDelete }: {
  visible: boolean; onClose: () => void; onReport: () => void; onBlock: () => void; onDelete: () => void;
}) => (
...
type ReplyRowProps = { reply: Comment; onLongPress: () => void; onLike: () => void; onReply: () => void };
  ...
  <TouchableOpacity activeOpacity={0.85} onLongPress={onLongPress} delayLongPress={400} style={cs.replyRow}>
    ...
    <View style={{ flex: 1 }}>
      <View style={cs.commentBubble}>
        ...
        <RichText text={reply.text} style={cs.commentText} onHashtagPress={tag => Alert.alert('Hashtag', `#${tag}`)} onMentionPress={user => Alert.alert('Mention', `@${user}`)} />
      </View>
      <View style={cs.commentMeta}>
        <Text style={cs.commentTime}>{reply.time}</Text>
        <TouchableOpacity onPress={onLike}><Text style={[cs.commentMetaBtn, reply.liked && cs.commentMetaActive]}>{reply.liked ? '❤️' : '🤍'} {reply.likes > 0 ? reply.likes : ''}</Text></TouchableOpacity>
        <TouchableOpacity onPress={onReply}><Text style={cs.commentMetaBtn}>Reply</Text></TouchableOpacity>
      </View>
    </View>
  </TouchableOpacity>
);
...
const CommentSheet = ({ visible, onClose }: { visible: boolean; onClose: () => void }) => {
  ...
  const handleLongPress = (id: string, parentId?: string) => { setCtxTarget({ id, parentId }); setCtxVisible(true); };
  const handleCtxClose = () => { setCtxVisible(false); setCtxTarget(null); };
  const handleCtxDelete = () => {
    handleCtxClose();
    if (!ctxTarget) return;
    if (ctxTarget.parentId) {
      setComments(prev => prev.map(c => c.id === ctxTarget.parentId ? { ...c, replies: c.replies.filter(r => r.id !== ctxTarget.id) } : c));
    } else {
      setComments(prev => prev.filter(c => c.id !== ctxTarget.id));
    }
  };

  const toggleCommentLike = (id: string) => setComments(prev => prev.map(c => c.id === id ? { ...c, liked: !c.liked, likes: c.liked ? c.likes - 1 : c.likes + 1 } : c));
  const toggleReplyLike = (parentId: string, replyId: string) => setComments(prev => prev.map(c => c.id === parentId ? { ...c, replies: c.replies.map(r => r.id === replyId ? { ...r, liked: !r.liked, likes: r.liked ? r.likes - 1 : r.likes + 1 } : r) } : c));

  const submitComment = () => {
    if (!newComment.trim()) return;
    const entry: Comment = { id: `c${Date.now()}`, user: 'You', avatar: 'https://i.pravatar.cc/150?img=47', text: newComment.trim(), time: 'now', likes: 0, liked: false, replies: [] };
    if (replyingTo) {
      const targetId = replyingTo.parentId ?? replyingTo.id;
      setComments(prev => prev.map(c => c.id === targetId ? { ...c, replies: [...c.replies, entry] } : c));
  ...
  const startReply = (id: string, user: string, parentId?: string) => { setReplyingTo({ id, user, parentId }); setTimeout(() => inputRef.current?.focus(), 100); };
  const toggleReplies = (id: string) => setExpandedReplies(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next; });
    ...
    <TouchableOpacity activeOpacity={0.85} onLongPress={() => handleLongPress(item.id)} delayLongPress={400} style={cs.commentRow}>
      ...
      <View style={cs.commentBody}>
        <View style={cs.commentBubble}>
          ...
          <RichText text={item.text} style={cs.commentText} onHashtagPress={tag => Alert.alert('Hashtag', `#${tag}`)} onMentionPress={user => Alert.alert('Mention', `@${user}`)} />
        </View>
        <View style={cs.commentMeta}>
          <Text style={cs.commentTime}>{item.time}</Text>
          <TouchableOpacity onPress={() => toggleCommentLike(item.id)}><Text style={[cs.commentMetaBtn, item.liked && cs.commentMetaActive]}>{item.liked ? '❤️' : '🤍'} {item.likes > 0 ? item.likes : ''}</Text></TouchableOpacity>
          <TouchableOpacity onPress={() => startReply(item.id, item.user)}><Text style={[cs.commentMetaBtn, replyingTo?.id === item.id && !replyingTo?.parentId && cs.commentMetaActive]}>Reply</Text></TouchableOpacity>
        </View>
        {item.replies.length > 0 && (
          <TouchableOpacity onPress={() => toggleReplies(item.id)} style={cs.viewRepliesBtn}>
            <View style={cs.viewRepliesLine} />
            <Text style={cs.viewReplies}>{expandedReplies.has(item.id) ? `▲ Hide ${item.replies.length} ${item.replies.length === 1 ? 'reply' : 'replies'}` : `▼ View ${item.replies.length} ${item.replies.length === 1 ? 'reply' : 'replies'}`}</Text>
          </TouchableOpacity>
        )}
        {expandedReplies.has(item.id) && item.replies.map(reply => (
          <ReplyRow key={reply.id} reply={reply} onLongPress={() => handleLongPress(reply.id, item.id)} onLike={() => toggleReplyLike(item.id, reply.id)} onReply={() => startReply(reply.id, reply.user, item.id)} />
        ))}
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <>
      <Modal transparent visible={visible} animationType="none" onRequestClose={onClose} statusBarTranslucent>
        <KeyboardAvoidingView style={cs.modalContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
          <Animated.View style={[cs.backdrop, { opacity: backdropAnim }]} pointerEvents={visible ? 'auto' : 'none'}>
            <Pressable style={{ flex: 1 }} onPress={onClose} />
          </Animated.View>
          <Animated.View style={[cs.sheet, { transform: [{ translateY: slideAnim }] }]}>
            <View style={cs.handle} />
            <Text style={cs.sheetTitle}>Comments</Text>
            <FlatList data={comments} keyExtractor={c => c.id} renderItem={renderComment} contentContainerStyle={cs.commentListContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" />
            <View style={cs.inputWrapper}>
              {replyingTo && (
                <View style={cs.replyingBanner}>
                  <Text style={cs.replyingText}>Replying to <Text style={cs.replyingUser}>{replyingTo.user}</Text></Text>
                  <TouchableOpacity onPress={() => setReplyingTo(null)}><Text style={cs.replyingCancel}>✕ Cancel</Text></TouchableOpacity>
                </View>
              )}
              <View style={cs.inputRow}>
                <Image source={{ uri: 'https://i.pravatar.cc/150?img=47' }} style={cs.inputAvatar} />
                <TextInput ref={inputRef} style={cs.commentInput} placeholder={replyingTo ? `Reply to ${replyingTo.user}...` : 'Add a comment...'} placeholderTextColor="#aaa" value={newComment} onChangeText={setNewComment} multiline />
                <TouchableOpacity onPress={submitComment} disabled={!newComment.trim()}><Text style={[cs.sendBtn, newComment.trim() ? cs.sendBtnActive : {}]}>Send</Text></TouchableOpacity>
              </View>
            </View>
          </Animated.View>
        </KeyboardAvoidingView>
      </Modal>
      <ContextMenu visible={ctxVisible} onClose={handleCtxClose} onReport={() => { handleCtxClose(); Alert.alert('Reported'); }} onBlock={() => { handleCtxClose(); Alert.alert('Blocked'); }} onDelete={handleCtxDelete} />
    </>
  );
};
...
const AnimatedLikeButton = ({ liked, likes, onPress }: { liked: boolean; likes: number; onPress: () => void }) => {
...
const PostActions = ({ liked, likeCount, comments, shares, onLike, onComment, onShare, onSave }: {
  liked: boolean; likeCount: number; comments: number; shares: number;
...
const AdCard = ({ post, onComment, onShare, onOptions, onSave }: { post: Post; onComment: () => void; onShare: () => void; onOptions: () => void; onSave: () => void }) => {
  ...
  return (
    <View style={styles.postCard}>
      <View style={styles.postHeader}>
        ...
        <TouchableOpacity style={styles.moreBtn} onPress={onOptions}><Text style={styles.moreDots}>⋮</Text></TouchableOpacity>
      </View>
      <RichText text={post.text} style={styles.postText} onHashtagPress={tag => Alert.alert('Hashtag', `#${tag}`)} onMentionPress={user => Alert.alert('Mention', `@${user}`)} />
      ...
      <View style={styles.adOfferRow}>
        ...
        <TouchableOpacity style={styles.adCtaBtn}><Text style={styles.adCtaText}>{post.adCta}</Text></TouchableOpacity>
      </View>
      <PostActions liked={liked} likeCount={likeCount} comments={post.comments} shares={post.shares} onLike={handleLike} onComment={onComment} onShare={onShare} onSave={onSave} />
    </View>
  );
};
...
const getMediaHeight = (aspect?: MediaAspect): number => {
  switch (aspect) {
    case 'portrait':  return Math.min(width * (16 / 9), 520);
    case 'square':    return width;
    case 'wide':      return width * (1 / 2.39);
    case 'landscape':
    default:          return width * (9 / 16);
  }
};

// ─── VideoPlayer ──────────────────────────────────────────────────────────────

const VideoPlayer = ({ uri, thumbnail, aspect, isActive, onDoubleTap }: { uri: string; thumbnail?: string; aspect?: MediaAspect; isActive: boolean; onDoubleTap: () => void }) => {
  ...
  return (
    <TouchableOpacity activeOpacity={1} onPress={handleTap} style={[vpStyles.container, { height: mediaH }]}>
      <Video source={{ uri }} style={StyleSheet.absoluteFill} resizeMode="cover" poster={thumbnail} paused={!playing} muted={false} repeat={true}
        onProgress={({ currentTime, seekableDuration }) => setProgress(seekableDuration > 0 ? currentTime / seekableDuration : 0)} />
...
const vpStyles = StyleSheet.create({
  container: { width: '100%', backgroundColor: '#000', overflow: 'hidden', position: 'relative' },
  progressTrack: { height: 3, overflow: 'hidden', position: 'absolute', bottom: 0, left: 0, right: 0 },
  progressFill: { height: '100%', backgroundColor: 'rgba(255,167,87,1)', borderRadius: 2 },
});

// ─── PostCard ─────────────────────────────────────────────────────────────────

const PostCard = ({ post, activePostId, onComment, onShare, onOptions, onSave }: { post: Post; activePostId: string | null; onComment: () => void; onShare: () => void; onOptions: () => void; onSave: () => void }) => {
  ...
  const triggerDoubleTapHeart = () => {
    ...
    heartScaleAnim.setValue(0); heartOpacityAnim.setValue(1);
    Animated.sequence([
      Animated.spring(heartScaleAnim, { toValue: 1, useNativeDriver: true, tension: 150, friction: 5 }),
      Animated.delay(600),
  ...
  const handleImageTap = () => {
    const now = Date.now();
    if (now - lastTapRef.current < 300) triggerDoubleTapHeart();
  ...
  return (
    <View style={styles.postCard}>
      ...
      {post.video ? (
        <View>
          <VideoPlayer uri={post.video} thumbnail={post.videoThumbnail} aspect={post.mediaAspect} isActive={activePostId === post.id} onDoubleTap={triggerDoubleTapHeart} />
          {doubleTapHeart && (
            <View style={pcStyles.heartOverlay} pointerEvents="none">
              <Animated.Text style={[pcStyles.heartEmoji, { opacity: heartOpacityAnim, transform: [{ scale: heartScaleAnim }] }]}>❤️</Animated.Text>
            </View>
          )}
          {post.views && <View style={styles.postViewsBadge}><Text style={styles.postViews}>👁 {post.views} Views</Text></View>}
        </View>
      ) : post.images.length > 0 ? (
        <View style={[styles.postImageWrapper, { height: getMediaHeight(post.mediaAspect) }]} onLayout={e => setCarouselWidth(e.nativeEvent.layout.width)}>
          <FlatList
            data={post.images} keyExtractor={(_, idx) => String(idx)} horizontal pagingEnabled
            showsHorizontalScrollIndicator={false} bounces={false} decelerationRate="fast"
            snapToInterval={carouselWidth} snapToAlignment="start" disableIntervalMomentum
            getItemLayout={(_, index) => ({ length: carouselWidth, offset: carouselWidth * index, index })}
            onMomentumScrollEnd={e => setCurrentImage(Math.round(e.nativeEvent.contentOffset.x / carouselWidth))}
            renderItem={({ item: img }) => (
              <Pressable onPress={handleImageTap} style={{ width: carouselWidth }}>
                <Image source={{ uri: img }} style={{ width: carouselWidth, height: getMediaHeight(post.mediaAspect) }} resizeMode="cover" />
              </Pressable>
            )}
          />
          {doubleTapHeart && (
          ...
          {post.views && <View style={styles.postViewsBadge}><Text style={styles.postViews}>👁 {post.views} Views</Text></View>}
          {post.images.length > 1 && <View style={styles.photoCountBadge}><Text style={styles.photoCountText}>{currentImage + 1}/{post.images.length}</Text></View>}
          {post.images.length > 1 && (
            <View style={pcStyles.dotsRow}>
              {post.images.map((_, idx) => <View key={idx} style={[pcStyles.dot, idx === currentImage && pcStyles.dotActive]} />)}
            </View>
          )}
        </View>
      ) : null}

      <PostActions liked={liked} likeCount={likeCount} comments={post.comments} shares={post.shares}
        onLike={() => { setLiked(p => { setLikeCount(c => p ? c - 1 : c + 1); return !p; }); }}
        onComment={onComment} onShare={onShare} onSave={onSave} />
    </View>
  );
};
...
const pcStyles = StyleSheet.create({
  ...
  heartEmoji: { fontSize: 90, textShadowColor: 'rgba(0,0,0,0.3)', textShadowOffset: { width: 0, height: 4 }, textShadowRadius: 12 },
  dotsRow: { position: 'absolute', bottom: 10, left: 0, right: 0, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 5, zIndex: 5 },
...
const StoryItem = ({ group, groupIndex, navigation }: { group: typeof STORY_GROUPS[0]; groupIndex: number; navigation: any }) => (
  <TouchableOpacity onPress={() => navigation.navigate('StoryView', { groupIndex, mode: group.isOwn && group.slides.length === 0 ? 'crop' : 'view' })} style={styles.storyItem} activeOpacity={0.8}>
    <View style={styles.storyRing}>
      <Image source={{ uri: group.userAvatar }} style={styles.storyAvatar} />
      {group.isOwn && (
        <TouchableOpacity onPress={e => { e.stopPropagation(); navigation.navigate('StoryView', { groupIndex, mode: 'crop' }); }} style={styles.storyAddBtn}>
...
const PeopleCard = ({ person, navigation }: { person: typeof PEOPLE_YOU_MAY_KNOW[0]; navigation: any }) => {
  ...
  return (
    <View style={styles.peopleCard}>
      ...
      <TouchableOpacity style={[styles.peopleActionBtn, person.status === 'Invite' ? styles.inviteBtn : styles.addBtn, added && styles.addedBtn]} onPress={() => setAdded(true)}>
        <Text style={[styles.peopleActionText, added && styles.addedBtnText]}>{added ? 'Added' : person.status}</Text>
      </TouchableOpacity>
...
type FeedItem = { kind: 'post'; data: Post } | { kind: 'ad'; data: Post } | { kind: 'people' };
...
const HomeScreen = ({ navigation }: { navigation?: any }) => {
  ...
  const [showBloodCard, setShowBloodCard] = useState(true);
  ...
  const onViewableItemsChanged = useRef(({ viewableItems }: any) => {
    const videoItem = viewableItems.find((v: any) => v.item?.kind === 'post' && v.item?.data?.video);
    ...
    if (item.kind === 'people') {
      return (
        <View style={styles.peopleSection}>
          ...
          <FlatList data={PEOPLE_YOU_MAY_KNOW} keyExtractor={p => p.id} renderItem={({ item: p }) => <PeopleCard person={p} navigation={navigation} />} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.peopleList} />
        </View>
      );
    }
    if (item.kind === 'ad') {
      return <AdCard post={item.data} onComment={() => setCommentPostId(item.data.id)} onShare={() => handleShare(item.data)} onOptions={() => setOptionsPost(item.data)} onSave={() => handleSave(item.data)} />;
    }
    return <PostCard post={item.data} activePostId={activePostId} onComment={() => setCommentPostId(item.data.id)} onShare={() => handleShare(item.data)} onOptions={() => setOptionsPost(item.data)} onSave={() => handleSave(item.data)} />;
  };
  ...
  const ListHeader = () => (
    <>
      ...
      {showBloodCard && (
        <BloodDonationBanner
          onIgnore={() => setShowBloodCard(false)}
          onAccept={() => { Alert.alert('Accepted', 'You accepted the blood donation request!'); setShowBloodCard(false); }}
        />
      )}

      {/* ── Birthday Banner (swipeable multi-card) ── */}
      {showBirthdayCard && (
        <BirthdayBanner onAllDone={() => setShowBirthdayCard(false)} />
      )}

      {/* ── Stories ── */}
      <FlatList
        data={STORY_GROUPS} keyExtractor={g => g.userId}
        renderItem={({ item, index }) => <StoryItem group={item} groupIndex={index} navigation={navigation} />}
      ...
      <View style={styles.mindBar}>
        <TouchableOpacity onPress={() => navigation?.navigate('CreatePost', { isHivePost: false, add: 0 })} style={styles.mindInput} activeOpacity={0.7}>
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <FlatList
        data={FEED_ITEMS}
        keyExtractor={(item, i) => item.kind === 'people' ? 'people' : item.data.id}
      ...
      <PostOptionsSheet visible={optionsPost !== null} onClose={() => setOptionsPost(null)} onSave={() => { if (optionsPost) handleSave(optionsPost); }} onShare={() => optionsPost && handleShare(optionsPost)} />
